  ...
  const fetchReviews = async () => {
    try {
      ...
      const [aggRes, listRes] = await Promise.all([
        supabase
          .from('profiles')
          .select('merchant_average_rating, merchant_reviews_count')
          .eq('id', user?.id)
          .single(),
        supabase
          .from('reviews')
      ...
      const aggAvg = Number(aggRes?.data?.merchant_average_rating ?? 0) || 0;
      const aggCnt = Number(aggRes?.data?.merchant_reviews_count ?? 0) || 0;
