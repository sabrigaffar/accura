...
-- This version is robust to schema variations (may not have earned_at)
...
BEGIN
  ...
  ELSIF v_has_created THEN
    ...
    ';
  ELSE
    -- No timestamp columns found, dedupe arbitrarily by ctid
    EXECUTE '
      WITH ranked AS (
        SELECT ctid, driver_id, order_id,
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.driver_earnings ADD CONSTRAINT uq_driver_earnings_driver_order UNIQUE (driver_id, order_id)';
  END IF;
END $$;
