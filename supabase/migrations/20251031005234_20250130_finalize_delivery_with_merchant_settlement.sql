-- ========================================
-- Extend finalize_delivery_tx to settle merchant + platform wallets (idempotent)
-- Handles card (visa) and cash uniformly with idempotency guards
-- ========================================
...
BEGIN
  ...
  v_merchant_amount := COALESCE(
    v_order.merchant_amount,
    COALESCE(v_order.product_total,0) + COALESCE(v_order.tax_amount,0),
    GREATEST(COALESCE(v_order.customer_total,0) - COALESCE(v_order.delivery_fee,0) - COALESCE(v_order.service_fee,0) - COALESCE(v_order.tax_amount,0), 0)
  );
  ...
  -- If already delivered: ensure earnings row exists and exit gracefully
  IF v_order.status = 'delivered' THEN
    ...
    IF NOT v_earning_exists THEN
      INSERT INTO driver_earnings(driver_id, order_id, amount, earned_at) VALUES (v_uid, p_order_id, v_driver_amount, now())
      ON CONFLICT (order_id) DO NOTHING;
    END IF;
    RETURN QUERY SELECT true, 'الطلب مسجَّل كمسلَّم سابقاً'::text; RETURN;
  END IF;

  -- Idempotent driver credit
  ...
  -- Platform capture for card payments (optional idempotent)
  IF COALESCE(v_order.payment_method,'') IN ('card','visa','online') THEN
    ...
    IF NOT v_platform_capture_exists THEN
      INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
      VALUES (v_platform_wallet, 'capture', COALESCE(v_order.customer_total, v_driver_amount + v_merchant_amount + v_platform_fee), 'Card capture', p_order_id, now());
      -- Platform keeps only its fee; the rest was transferred to merchant/driver as internal settlement
      UPDATE wallets SET balance = balance + v_platform_fee, updated_at = now() WHERE id = v_platform_wallet;
    END IF;
  ELSE
    -- Cash: platform collects only service fee if applicable (idempotent via adjust/capture)
    SELECT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_platform_wallet AND related_order_id=p_order_id AND type='capture') INTO v_platform_capture_exists;
