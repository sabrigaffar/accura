-- Enforce driver minimum balance (50 EGP) at accept time
-- Add driver_can_accept(min_balance) helper and update accept_order_safe to use dynamic per-km/base settings
-- Date: 2025-11-10

BEGIN;

-- Helper: check if current driver can accept based on available balance (balance - active holds)
...
BEGIN
  ...
  IF v_available < p_min THEN
    allowed := false;
    message := format('رصيد محفظتك المتاح %.2f أقل من الحد الأدنى %.2f. لا يمكنك قبول الطلب.', v_available, p_min);
    RETURN NEXT; RETURN;
  END IF;
...
-- Update accept_order_safe to (1) gate by driver_can_accept(50) and (2) compute hold using dynamic base and per-km commission
...
BEGIN
  -- Gate by driver_can_accept (minimum available balance 50)
