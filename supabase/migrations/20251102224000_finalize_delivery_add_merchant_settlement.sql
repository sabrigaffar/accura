-- Add merchant settlement (product + tax) to finalize_delivery_tx and card capture for online
-- Idempotent credits; preserves driver capture debit logic
-- Date: 2025-11-02
...
begin
  ...
  -- 1) Credit driver earnings (delivery_fee) to wallet (idempotent)
  select exists(
  ...
  -- 1.b) Record/ensure driver_earnings row with commission and net calculated
  select exists(
  ...
  -- 1.c) Credit merchant settlement (product + tax) for ONLINE ONLY, idempotent
  ...
  if v_payment_method = 'cash' then
    ...
    -- Online (card/wallet): capture km_fee only from driver; service_fee via card capture below
    if v_hold.id is not null then
      update public.wallet_holds set status = 'captured', captured_at = v_now where id = v_hold.id;
      -- Debit driver wallet by km_fee
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
    ...
    -- Card capture accounting (idempotent)
    select exists(
      select 1 from public.wallet_transactions wt
    ...
    if not v_platform_capture_exists then
      ...
      -- Platform increases balance only by service_fee
