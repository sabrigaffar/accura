-- Fix finalize_delivery_tx to credit the merchant USER wallet (per-user model)
-- Previously it used orders.merchant_id (store id) which could create/credit a store-level wallet
-- Date: 2025-11-06
...
declare
  ...
  v_merchant_user uuid := null;  -- NEW: owner_id of the merchant (user level)
begin
  ...
  if v_merchant_user is not null then
    ...
    RAISE NOTICE 'finalize_delivery_tx: merchant_user=% wallet_id=%', v_merchant_user, v_merchant_wallet;
  else
    RAISE NOTICE 'finalize_delivery_tx: merchant owner_id is NULL!';
  end if;
  ...
  -- Policy change: For CASH, driver pays per-km only (no service fee deduction from driver)
  ...
  if v_payment_method = 'cash' then
    if v_hold.id is not null then
      ...
      -- Capture only per-km from driver for CASH, release any excess hold
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
      values (v_driver_wallet, 'capture', v_km_fee, 'تحصيل رسوم المنصة لكل كم (CASH)', p_order_id);
      ...
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
      values (v_admin_wallet, 'transfer_in', v_km_fee, 'رسوم المنصة لكل كم (CASH)', p_order_id);
