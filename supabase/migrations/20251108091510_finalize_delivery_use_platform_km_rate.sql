-- Use platform_settings.driver_commission_per_km for per-km platform fee
-- Compute km_fee = ceil(distance_km) * driver_commission_per_km
-- Fallback when distance missing derives kilometers from delivery_fee/base_rate (base_rate=10)
-- Keeps CASH policy from 20251107000003 (no driver deposit on cash, capture hold (km+service))
...
DECLARE
  ...
  v_base_fee_per_km numeric := 10; -- base used by calculate_delivery_fee
...
BEGIN
  ...
  -- Resolve platform per-km rate
  SELECT COALESCE(driver_commission_per_km, 1) INTO v_per_km_driver FROM public.platform_settings WHERE id = 1;
  ...
  ELSE
    -- Derive from delivery fee and base per-km used in calculate_delivery_fee
