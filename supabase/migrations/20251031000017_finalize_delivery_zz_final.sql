-- Final override of finalize_delivery_tx to ensure latest logic is applied last in lexical order
-- Mirrors the version with cash policy and idempotent accounting
...
DECLARE
  ...
  v_platform_mc_exists boolean;
  v_platform_dc_exists boolean;
...
BEGIN
  ...
  SELECT 
    ...
    merchant_commission_apply_on_cash
  INTO v_settings
  FROM platform_settings WHERE id = 1;

  v_commission_per_km := COALESCE(v_settings.driver_commission_per_km, 1);
  v_commission_free_until := v_settings.driver_commission_free_until;
  ...
  -- merchant commission cash policy
  IF v_payment_method IN ('cash') AND COALESCE(v_settings.merchant_commission_apply_on_cash, false) = false THEN
    v_merchant_commission := 0;
  ELSE
    v_merchant_commission := GREATEST(ROUND(v_merchant_gross * (v_merchant_commission_rate/100.0), 2) + v_merchant_commission_flat, 0);
  ...
  IF v_commission_free_until IS NULL OR now() > v_commission_free_until THEN
    ...
    IF v_driver_commission > v_driver_amount THEN
      v_driver_commission := v_driver_amount;
    END IF;
  ELSE
  ...
  -- already delivered? upsert and commissions then exit
  IF v_order.status = 'delivered' THEN
    ...
    IF v_driver_commission > 0 THEN
      IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_driver_wallet AND related_order_id=p_order_id AND type='transfer_out' AND memo='Driver per-km commission') THEN
      ...
      END IF;
      IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_platform_wallet AND related_order_id=p_order_id AND type='transfer_in' AND memo='Driver per-km commission') THEN
        INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
    ...
    IF v_merchant_commission > 0 THEN
      IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_platform_wallet AND related_order_id=p_order_id AND type='transfer_in' AND memo='Merchant commission') THEN
        INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
  ...
  -- driver credit
  IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_driver_wallet AND related_order_id=p_order_id AND type='transfer_in') THEN
    INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
  ...
  -- merchant credit (net)
  IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_merchant_wallet AND related_order_id=p_order_id AND type='transfer_in') THEN
    INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
  ...
  -- platform capture & fees
  IF v_payment_method IN ('card','visa','online') THEN
    IF NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_platform_wallet AND related_order_id=p_order_id AND type='capture') THEN
      INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
    ...
    END IF;
  ELSE
    IF v_platform_fee > 0 AND NOT EXISTS(SELECT 1 FROM wallet_transactions WHERE wallet_id=v_platform_wallet AND related_order_id=p_order_id AND type='capture') THEN
      INSERT INTO wallet_transactions(wallet_id, type, amount, memo, related_order_id, created_at)
  ...
  RETURN QUERY SELECT true, 'تم إنهاء التسليم وتسوية المستحقات والعمولات (نسخة نهائية)'::text; RETURN;
END $$;
