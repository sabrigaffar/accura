-- Update finalize_delivery_tx: capture expected (km_fee + service_fee) for CASH, release difference if hold > expected
-- Keeps dynamic per-km rate and base per-km fee from platform_settings
-- Date: 2025-11-09
...
BEGIN
  ...
  SELECT id, amount, status INTO v_hold FROM public.wallet_holds WHERE related_order_id = p_order_id AND status = 'active' LIMIT 1;
