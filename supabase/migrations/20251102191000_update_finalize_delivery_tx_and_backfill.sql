-- Update finalize_delivery_tx to compute delivery fee with fallbacks
-- and backfill existing driver_earnings amounts that are 0/null
BEGIN;
...
-- Backfill existing zero/NULL amounts in driver_earnings from orders
WITH derived AS (
  SELECT de.id AS earning_id,
         GREATEST(
  ...
  WHERE de.amount IS NULL OR de.amount = 0
)
UPDATE public.driver_earnings de
SET amount = d.amount_fix,
    net_amount = COALESCE(de.net_amount, d.amount_fix),
    commission_amount = COALESCE(de.commission_amount, 0),
    earned_at = COALESCE(de.earned_at, de.created_at, now())
FROM derived d
WHERE de.id = d.earning_id;
