-- Resilient push dispatch: try both extensions.http_post and net.http_post with different signatures, swallow undefined_function
-- Date: 2025-11-11

BEGIN;

-- Ensure pg_net exists (prefer extensions schema on Supabase)
DO $$
BEGIN
...
-- Drop trigger first to avoid dependency when replacing the function
...
BEGIN
  FOR rec IN (
    ...
    -- Try multiple variants safely; ignore if function is missing
    BEGIN
      -- 1) Supabase typical: extensions.http_post(text, jsonb, text)
    ...
    EXCEPTION WHEN undefined_function THEN
      BEGIN
        -- 2) net.http_post(text, jsonb, text)
      ...
      EXCEPTION WHEN undefined_function THEN
        BEGIN
          -- 3) extensions.http_post(text, jsonb, jsonb)
          PERFORM extensions.http_post(
            'https://exp.host/--/api/v2/push/send'::text,
            jsonb_build_object('Content-Type','application/json'),
            jsonb_build_object(
        ...
        EXCEPTION WHEN undefined_function THEN
          -- 4) net.http_post(text, jsonb, jsonb)
          BEGIN
            PERFORM net.http_post(
              'https://exp.host/--/api/v2/push/send'::text,
              jsonb_build_object('Content-Type','application/json'),
          ...
          EXCEPTION WHEN undefined_function THEN
            -- Give up silently (do not block business flow)
            NULL;
          END;
        END;
      END;
