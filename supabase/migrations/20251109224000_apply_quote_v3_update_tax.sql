-- Update apply_quote_v3_to_order to persist tax from quote
-- Date: 2025-11-09
...
BEGIN
  ...
  v_store_id := v_order.merchant_id;
  ...
  -- Update order to match quote breakdown (including tax)
  ...
  SET subtotal = COALESCE(v_q.subtotal, subtotal),
      ...
      tax = COALESCE(v_q.tax, tax),
      total = COALESCE(v_q.total, total),
      updated_at = now()
  WHERE id = p_order_id;

  -- Remove prior auto-calculated discount rows to avoid duplication
  ...
  RETURN QUERY SELECT true, 'تم تطبيق تسعير الكوت بشكل نهائي على الطلب وتحديث الضريبة'::text; RETURN;
END;
$$;
