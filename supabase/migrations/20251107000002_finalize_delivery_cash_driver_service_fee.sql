-- Revert CASH policy: driver pays service_fee + per-km (km_fee) on delivery
-- Keep ONLINE policy: driver pays per-km only; platform captures card amount and service_fee separately
-- Also keep merchant settlement to USER wallet (owner_id of merchant) for ONLINE payments only
-- Date: 2025-11-07
...
begin
  ...
  -- 1) Credit driver earnings (deposit delivery fee to driver)
  select exists(
  ...
  if v_payment_method in ('card','wallet','online','visa') then
    RAISE NOTICE 'finalize_delivery_tx: Payment is online, checking merchant settlement...';
  ...
  if v_payment_method = 'cash' then
    ...
    if v_hold.id is not null then
      update public.wallet_holds set status = 'captured', captured_at = v_now where id = v_hold.id;
      -- ONLINE: capture per-km from driver hold
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
