-- Merchant-level tax rate setting and dynamic tax in quote_order_v3
-- Date: 2025-11-09

BEGIN;

-- 1) Add per-merchant tax rate percent (0..100). Default 0 (no tax)
ALTER TABLE public.merchants
  ADD COLUMN IF NOT EXISTS tax_rate_percent numeric NOT NULL DEFAULT 0;

-- 2) Recreate quote_order_v3 to compute tax from merchant setting instead of client-supplied p_tax
CREATE OR REPLACE FUNCTION public.quote_order_v3(
  ...
  p_tax numeric DEFAULT 0,  -- kept for backward-compat; ignored in favor of merchant tax
...
DECLARE
  ...
  v_tax numeric := 0;
BEGIN
  -- Step A: compute base quote WITHOUT tax first
  SELECT * INTO v_q FROM public.calculate_order_quote_v2(
    ...
    p_payment_method,
    p_delivery_fee,
    0 -- force zero tax inside v2; we will compute tax here from merchant setting
  );
  ...
  -- Step B: compute tax from merchant setting (percentage of subtotal)
  SELECT COALESCE(tax_rate_percent, 0) INTO v_tax_rate FROM public.merchants WHERE id = p_store_id;
  v_tax := ROUND(GREATEST(v_subtotal, 0) * (COALESCE(v_tax_rate, 0) / 100.0), 2);

  -- Step C: consider Ad discount (best of with rule/promo)
  ...
  SELECT 
    ...
    v_subtotal + v_df + v_service_fee + v_tax - COALESCE(v_best_discount,0),
