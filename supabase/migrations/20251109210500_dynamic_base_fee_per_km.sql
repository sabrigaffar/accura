-- Migration: Make base per-km delivery fee dynamic from platform_settings
-- Date: 2025-11-09

BEGIN;

-- 1) Add base_fee_per_km to platform_settings
ALTER TABLE public.platform_settings
  ADD COLUMN IF NOT EXISTS base_fee_per_km numeric NOT NULL DEFAULT 10;
...
SET base_fee_per_km = COALESCE(base_fee_per_km, 10)
WHERE id = 1;

-- 2) Update calculate_delivery_fee(distance_km) to use platform base_fee_per_km and ceil(distance)
...
BEGIN
  SELECT COALESCE(base_fee_per_km, 10) INTO v_base_fee_per_km FROM public.platform_settings WHERE id = 1;
  v_kilometers := CEIL(COALESCE(distance_km, 0));
  RETURN GREATEST(v_kilometers, 0) * v_base_fee_per_km;
END;
$$;

-- 3) Update finalize_delivery_tx to use dynamic base_fee_per_km when deriving distance from fee
...
BEGIN
  ...
  -- Resolve platform per-km rate and base per-km fee
  SELECT COALESCE(driver_commission_per_km, 1), COALESCE(base_fee_per_km, 10) INTO v_per_km_driver, v_base_fee_per_km FROM public.platform_settings WHERE id = 1;

  -- Determine kilometers for km_fee
  ...
  IF v_payment_method = 'cash' THEN
    IF v_hold.id IS NOT NULL THEN
      ...
      -- Capture full hold (km_fee + service_fee) from driver for CASH
      INSERT INTO public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
...
-- 4) Public RPC to read base_fee_per_km (safe to expose)
CREATE OR REPLACE FUNCTION public.get_base_fee_per_km()
RETURNS NUMERIC
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_base NUMERIC := 10;
BEGIN
  SELECT COALESCE(base_fee_per_km, 10) INTO v_base FROM public.platform_settings WHERE id = 1;
  RETURN v_base;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_base_fee_per_km() TO anon, authenticated;
