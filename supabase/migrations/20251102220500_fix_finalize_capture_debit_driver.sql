-- Fix finalize_delivery_tx to debit driver wallet on capture (per-km and COD service fee)
-- So driver wallet balance reflects net after platform captures
-- Date: 2025-11-02
...
begin
  ...
  if not v_exists then
    ...
    -- Attempt to harmonize legacy columns if present
    if exists (
  ...
  if v_payment_method = 'cash' then
    ...
    if v_hold.id is not null then
      ...
      -- Record capture on driver wallet and DEBIT driver balance
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
    ...
    if v_hold.id is not null then
      update public.wallet_holds set status = 'captured', captured_at = v_now where id = v_hold.id;
      -- Record capture on driver wallet and DEBIT driver balance by km_fee
      insert into public.wallet_transactions(wallet_id, type, amount, memo, related_order_id)
      ...
      -- Release any remaining amount from hold (if any)
